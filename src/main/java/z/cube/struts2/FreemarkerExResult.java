...
public class FreemarkerExResult extends StrutsResultSupport {
	...
	public FreemarkerExResult() {
		super();
	}

	public FreemarkerExResult(String location) {
		super(location);
	}
	...
	public void doExecute(String location, ActionInvocation invocation) throws IOException, TemplateException {
		...
		this.fileName = conditionalParse(fileName, invocation);
		this.staticTemplate = conditionalParse(staticTemplate, invocation);
		this.filePath = conditionalParse(filePath, invocation) == null ? "" : conditionalParse(
				filePath, invocation);
		...
		if (preTemplateProcess(template, model)) {
			try {
				...
				//需要在process输出内容到out之后
				try {
					String result = JSONUtil.serialize(out.toString());
